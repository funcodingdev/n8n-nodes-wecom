...
export const externalContactExtraHttpOps: ExtraHttpOp[] = [
	...
	{ id: 'crmAddMsgTemplate', name: '[客户联系(旧)] 创建企业群发', action: '创建企业群发（旧版路径）', description: '创建企业群发（旧版路径）', path: '/cgi-bin/crm/add_msg_template', method: 'POST' },
	{ id: 'crmGetCustomerContacts', name: '[客户联系(旧)] 获取客户列表', action: '获取客户列表（旧版路径）', description: '获取客户列表（旧版路径）', path: '/cgi-bin/crm/get_customer_contacts', method: 'POST' },
	{ id: 'crmGetExternalContact', name: '[客户联系(旧)] 获取客户详情', action: '获取客户详情（旧版路径）', description: '获取客户详情（旧版路径）', path: '/cgi-bin/crm/get_external_contact', method: 'POST' },
	{ id: 'crmGetExternalContactList', name: '[客户联系(旧)] 获取客户列表2', action: '获取客户列表（旧版路径）', description: '获取客户列表（旧版路径）', path: '/cgi-bin/crm/get_external_contact_list', method: 'POST' },
	{ id: 'crmGetGroupMsgResult', name: '[客户联系(旧)] 获取群发结果', action: '获取群发结果（旧版路径）', description: '获取群发结果（旧版路径）', path: '/cgi-bin/crm/get_group_msg_result', method: 'POST' },
	{ id: 'crmGetUnassignedList', name: '[客户联系(旧)] 获取待分配列表', action: '获取待分配列表（旧版路径）', description: '获取待分配列表（旧版路径）', path: '/cgi-bin/crm/get_unassigned_list', method: 'POST' },
	{ id: 'crmGetUserBehaviorData', name: '[客户联系(旧)] 获取联系客户统计', action: '获取联系客户统计（旧版路径）', description: '获取联系客户统计（旧版路径）', path: '/cgi-bin/crm/get_user_behavior_data', method: 'POST' },
	{ id: 'crmTransferExternalContact', name: '[客户联系(旧)] 分配客户', action: '分配客户（旧版路径）', description: '分配客户（旧版路径）', path: '/cgi-bin/crm/transfer_external_contact', method: 'POST' },
];
