...
export const meetingAdvancedOpsDescription: INodeProperties[] = [
	...
	{
		displayName: '获取布局模板列表为 GET 接口，无需请求参数。返回 layout_template_list，可用于添加布局时填写模板 ID。',
	...
	},
	{
		...
		type: 'boolean',
		displayOptions: { show: { resource: ['meeting'], operation: ['bookRooms'] } },
		default: true,
		description: 'subject_visible；仅影响会议开始前一小时内的 Rooms 展示',
	},
	// --- 会中扩展 ---
	{
		displayName: '联席主持人动作',
	...
	{
		displayName: '写入下方会中设置',
		name: 'realcontrol_write_form_fields',
		type: 'boolean',
		displayOptions: { show: { resource: ['meeting'], operation: ['realcontrolSet'] } },
		default: false,
		description: '开启后会把下方所有表单设置写入请求；关闭时仅使用扩展 JSON',
	},
	{
		displayName: '全体静音',
		name: 'rc_mute_all',
		type: 'boolean',
		displayOptions: { show: { resource: ['meeting'], operation: ['realcontrolSet'], realcontrol_write_form_fields: [true] } },
		default: false,
	},
	{
