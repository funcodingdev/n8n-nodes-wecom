---
title: "识别信息录入指令"
path: "004-智慧硬件开发/003-硬件云端接入/003-考勤-门禁设备/002-回调通知/001-识别信息录入指令"
specfusion_id: wecom_4bf103b00ba5
source_url: "https://developer.work.weixin.qq.com/document/path/96009"
last_updated: "2026-02-12T18:05:45.793Z"
calibrated_from: "SpecFusion"
calibrated_at: "2026-08-11T08:20:42Z"
---
**录入指纹或人脸指令**
...
| command | 结构体 | 指令数据 |
| command_type | string |指令类型，成员录入识别信息，此处固定为enter_page |
...
| msg_type | string | 用于表示本消息是 事件event 还是指令 command，此处固定为command , 对应command节点数据有效 |
| command | 结构体 | 指令数据 |
| command_type | string |指令类型，成员退出录入识别信息，此处固定为exit_page |
| page_type | string | 页面类型： "fa":录入脸部识别信息， "fp"：录入指纹信息 |
| oper_id | string | 本次操作的id |
| open_userid | string | 成员id（字符串） |
| user_type | uint32 | 成员的类型： 0：企业员工 2：学生 不填则默认值为0 |
