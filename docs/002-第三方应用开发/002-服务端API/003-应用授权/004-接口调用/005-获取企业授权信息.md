---
title: "获取企业授权信息"
path: "002-第三方应用开发/002-服务端API/003-应用授权/004-接口调用/005-获取企业授权信息"
specfusion_id: wecom_949683a78e4b
source_url: "https://developer.work.weixin.qq.com/document/path/100779"
last_updated: "2026-02-12T17:11:27.040Z"
calibrated_from: "SpecFusion"
calibrated_at: "2026-08-11T07:48:06Z"
---
**请求方式：**POST（**HTTPS**）
**请求地址：** https://qyapi.weixin.qq.com/cgi-bin/service/v2/get_auth_info?suite_access_token=SUITE_ACCESS_TOKEN
...
**参数说明：** 
 ...
 |auth_corp_info.corp_scale| 企业规模。当企业未设置该属性时，值为空。[成员授权](#30245)下，即auth_info.agent.auth_mode为1时值为空 |
 |auth_corp_info.corp_industry| 企业所属行业。当企业未设置该属性时，值为空。[成员授权](#30245)下，即auth_info.agent.auth_mode为1时值为空 |
 |auth_corp_info.corp_sub_industry| 企业所属子行业。当企业未设置该属性时，值为空。[成员授权](#30245)下，即auth_info.agent.auth_mode为1时值为空 |
 |auth_corp_info.corp_ex_name| 企业其他认证的名称，仅认证企业才有 |
 |auth_corp_info.corp_ex_name.name_list| 企业其他认证的企业简称列表（不包括corp_name），仅认证企业才有 |
 |auth_info|授权信息。<font color=red>如果是通讯录应用，且没开启实体应用，是没有该项的。通讯录应用拥有企业通讯录的全部信息读写权限</font>「第三方会话存档接口」不返回该字段|
 ...
 |auth_info.agent.privilege.level|权限等级。<br />1:通讯录基本信息只读<br />2:通讯录全部信息只读（已废弃）<br />3:通讯录全部信息读写<br />4:单个基本信息只读<br />5:通讯录全部信息只写（已废弃）<br />0:为代开发应用，该值无意义，固定为0，代开发应用权限可使用[获取应用权限](#48334)接口获取 |
 |auth_info.agent.shared_from|共享了应用的企业信息，仅当由[企业互联](#24909)或者[上下游](#35931)共享应用触发的安装时才返回|
 |auth_info.agent.shared_from.corpid|共享了应用的企业corpid，仅当[企业互联](#24909)或者[上下游](#35931)共享应用触发的安装时才返回|
 |auth_info.agent.shared_from.share_type|共享了途径，0表示企业互联，1表示上下游|
 |dealer_corp_info|[代理服务商](#15259)企业信息 |
 |dealer_corp_info.corpid|[代理服务商](#15259)企业微信id |
 |dealer_corp_info.corp_name|[代理服务商](#15259)企业微信名称 |

 >注：旧[获取企业授权信息](#14943)接口会获取企业微信插件关注二维码，生成二维码性能较差，若开发者不需要获取企业微信插件关注二维码，推荐使用该文档中v2接口。
