---
title: "添加记录"
path: "001-企业内部开发/002-服务端API/025-文档/010-接收外部数据到智能表格/002-添加记录"
specfusion_id: wecom_4d3072f17484
source_url: "https://developer.work.weixin.qq.com/document/path/101240"
last_updated: "2026-07-26T06:07:07.522Z"
calibrated_from: "SpecFusion"
calibrated_at: "2026-08-11T08:49:56Z"
---
## 添加记录
...
|日期(FIELD_TYPE_DATE_TIME)|string|日期单元格内容。</br>**时间戳模式**：传入以毫秒为单位的 unix 时间戳，如 `"1717228800000"`</br>**固定格式模式**：传入格式化字符串，支持 `yyyy-mm-dd hh:mm:ss` 或 `yyyy-mm-dd`，如 `"2024-06-01 12:00:00"`。默认时区为东八区|
|图片(FIELD_TYPE_IMAGE)|Object\[\]([CellImageValue](#cellimagevalue))|
|成员(FIELD_TYPE_USER)|string\[\] \| Object\[\]([CellUserValue](#celluservalue))|成员单元格内容。</br>**姓名(别名)模式**：传入字符串数组，支持输入企微人员姓名(别名)添加，如 `["张三","lisi","王五(wangwu)"]`。当前仅支持添加本企业内成员，在管理端可查询、设置当前企业成员的姓名(别名)</br>**userid 模式**：传入对象数组，如 `[{"user_id":"USERID"}]` |
...
|关联(FIELD_TYPE_REFERENCE)|string\[\] \| Object\[\]([CellReferenceValue](#cellreferencevalue))|关联到另一张表的记录。</br>**record_id 数组模式**：传入字符串数组，如 `["RECORDID1","RECORDID2"]`</br>**对象数组模式**：传入对象数组，如 `[{"record_id":"RECORDID1"},{"record_id":"RECORDID2"}]`</br>请求时支持两种模式写入，返回时默认为对象数组模式。字段属性设置为允许多选时可设置多个 record_id；每个 record_id 必须在被关联表中存在|
