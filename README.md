...
- 已注册为 n8n 的 Human review 渠道，可从 AI Agent 的工具连接器中直接添加企业微信人工审核步骤。
- 默认消息使用 `$tool.name` 和 `$tool.parameters` 展示 AI 即将调用的工具及完整参数；Date、DateRange、Selector、Contact、File 等工具入参会随工具调用自动进入审批内容。
- 配置 1～6 个“审批人可选操作”，并为每个操作设置按钮文字、唯一的工作流返回值和执行结果。
- 每个操作可设置按钮外观，以及选择后是否允许 AI 工具继续执行。
- “打开结果页”设置简单，适合大多数场景；“直接在企业微信中选择”可记录实际操作成员。
- 企业微信内提交后会把同一张审批卡片对所有接收人更新为不可重复点击的结果状态。
- 工作流结果保留 n8n 所需的 `approved`，并额外返回 `selectedOption` 和 `selectedLabel`。
- 选择“直接在企业微信中选择”时，需要在“企业微信消息接收”触发器中开启“自动处理企业微信内的审批选择”，且发送、接收凭证必须属于同一企业。
